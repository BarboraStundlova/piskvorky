...
/* začíná náhodný symbol
isPlaying = symbols[parseInt(Math.random() * 2)];
  ...
  btnsElm[i].addEventListener('click', () => {
    ...
    if (isWinningMove(btnsElm[i])) {
      setTimeout(() => {
        ...
        } else if (confirm('Vyhrálo kolečko')) {
          location.reload();
        }
      }, 200); //přidávám prodlevu před tím, než vyskočí ten prompt
    }
  });
}
//vyhra
...
const isWinningMove = (field) => {
  ...
  for (let i = 0; i < getCoordsLeanUp(column, row).count; i += 1) {
    leanUpSymbols.push(
      getSymbol(
        getField(
          getCoordsLeanUp(column, row).startY - i,
          getCoordsLeanUp(column, row).startX + i,
        ),
      ),
    );
  }
  ...
  for (let i = 0; i < getCoordsLeanDown(column, row).count; i += 1) {
    leanDownSymbols.push(
      getSymbol(
        getField(
          getCoordsLeanDown(column, row).startY + i,
          getCoordsLeanDown(column, row).startX + i,
        ),
      ),
    );
  }
  ...
  if (
    isWinning(rowSymbols) ||
    isWinning(columnSymbols) ||
    isWinning(leanUpSymbols) ||
    isWinning(leanDownSymbols)
  ) {
    return true;
  } else {
    return false;
